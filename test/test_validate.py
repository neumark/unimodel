...
class ValidationTestCase(TestCase):
    ...
    def nonworking_test_missing_required(self):
        ...
        self.assertRaises(
            ReadValidationException,
            lambda: ValidationTestClass.deserialize(
                s,
                ThriftProtocol('json').factory))
    ...
    def test_type_parameter_validator(self):
        ...
        class G(Unimodel):
            f = Field(
                List(Int(metadata=Metadata(validators=[ElementValidator()]))))
    ...
    def test_validate_map(self):
        ...
        class F(Unimodel):
            f = Field(Map(Int(metadata=Metadata(validators=[KeyValidator()])), List(
                UTF8, metadata=Metadata(validators=[ValueValidator()]))))
        # both key and value is OK
        F(f={2: ["a", "b", "c", "d"]}).validate()
        # validation of key fails
        self.assertRaises(
            ValidationException, lambda: F(f={1: ["a", "b", "c", "d"]}).validate())
        # validation of value fails
        self.assertRaises(
            ValidationException, lambda: F(f={4: ["a", "b", "c"]}).validate())
    ...
    def test_validate_deeply_nested(self):
        ...
        self.assertRaises(
            ValidationException, lambda: F(f=[[[{"a": [[3]]}]]]).validate())
